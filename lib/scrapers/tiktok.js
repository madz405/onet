...
// Beberapa field dari tikwm (misal "cover") kadang sudah berupa URL lengkap
// (langsung dari CDN TikTok), sementara field lain (misal "play"/"hdplay")
// berupa path relatif yang perlu ditambah domain tikwm.com. Kalau path
// relatif ditambah domain lagi tanpa dicek dulu, hasilnya jadi URL dobel
// yang rusak — makanya semua field URL di sini lewat helper ini dulu.
function absolutize(path) {
  if (!path) return null;
  return /^https?:\/\//i.test(path) ? path : "https://www.tikwm.com" + path;
}
...
export async function tiktokDl(url) {
  ...
  let media = [];
  if (data.duration == 0) {
    (data.images || []).forEach((v) => v && media.push({ type: "photo", url: absolutize(v) }));
  } else {
    media = [
      { type: "watermark", url: absolutize(data.wmplay) },
      { type: "nowatermark", url: absolutize(data.play) },
      { type: "nowatermark_hd", url: absolutize(data.hdplay) },
    ].filter((m) => m.url);
  }
  ...
  return {
    ...
    music_info: data.music_info
      ? {
          ...
          url: absolutize(data.music) || absolutize(data.music_info.play),
        }
      : null,
    stats: {
